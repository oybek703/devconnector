...
const EditProfile = ({history}) => {
    const [formData, setFormData] = useState({company: '', website: '', location: '', status: '', skills: '', bio: '', githubusername: '', youtube: '', twitter: '', facebook: '', linkedin: '', instagram: ''});
    const [socials, showSocials] = useState(false);
    const {loading, profile} = useSelector(state => state.profile);
    const dispatch = useDispatch();
    const {company, website, location, status, skills, bio, githubusername, youtube, twitter, facebook, linkedin, instagram} = formData;
    useEffect(() => {
    ...
    return (
        <Fragment>
            ...
            <form onSubmit={handleSubmit} className="form">
                ...
                <div className="my-2">
                    <button onClick={() => showSocials(!socials)} type="button" className="btn btn-light">
